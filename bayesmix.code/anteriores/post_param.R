...
post_param<-function(datos,param0,asigna,nom.varc,xjs_barra,k,d,folio=NULL){
  ...
  aux.media<-param0[c("v.k","variable","mu0")]
  
  aux.param<-distinct(param0[c("v.k","v","njs")])
  aux.param <- merge(aux.param,aux.xjs,id=v.k) 
  var.i<-as.matrix(aux.param[5:(4+d)])
  aux.vars<-data.frame(variable=rep(colnames(var.i),d,2*d,k),xj=matrix(var.i))
  ...
  aux.param <- data.frame(aux.param,nzj=(aux.param$njs*aux.param$zj)/aux.param$njs
                            +aux.param$zj,dif_m=aux.param$xj-aux.param$mu0)
  ...
  for(l in 1:k){
    ...
    if(indice!=0){
      ...
      diffm<-data.frame(diffm$dif_m[1],diffm$dif_m[2],diffm$dif_m[3])
      diffm <- setNames(diffm,mdifnom.varc)
      ...
      vars.y<-aux.vars[nom.varc]
      vars.y<-setNames(vars.y,ynom.varc)
  ...
  new.param<-data.frame(aux.param[1:2],
                        ...
                        media=(aux.param$mu0*aux.param$njs
