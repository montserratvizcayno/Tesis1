...
simulacion_mult <- function(datos, nom.var = names(datos), componente = 2,
                          folio = NULL, iteraciones = 1000){
  ##set.seed(9024)
  
  k <- componente
  param <- dist_incial_mult(datos, nom.var, k) ##devuelve los par?metros iniciales
  sims <- NULL
  sims_total <- NULL
  col <- as.numeric(which(names(datos) %in% nom.var))
  est <- estadisticos_iniciales(datos)
  mu0 <- data.frame(m=est[which(est$variable == nom.var), "media"]) #la media obtenida de los datos
  ...
  for(j in 1:iteraciones){
    ...
    for( i in 1:10){
      ...
      aux<-var_latente_mult(datos,nom.var = nom.var,k=k,folio = 'Cliente',
